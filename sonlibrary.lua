...
do
	...
	function SonLib:ResolveImage(path, instance, prop)
...
function SonLib:Gradient(text, fromColor, toColor)
	local colors
...
local ContentProvider = SonLib.GetService("ContentProvider")
local UserInputService = SonLib.GetService("UserInputService")
...
local function GetGui()
	...
	local parent = RunService:IsStudio() 
		...
		or (cloneref and cloneref(SonLib.GetService("CoreGui")) or SonLib.GetService("CoreGui"))
...
function SonLib:Window(Settings)
	...
	local Theme = {}
	do
		local src = SonLib.Themes[Settings.Theme or SonLib.CurrentTheme] or {}
	...
	function WindowFunctions:TabGroup()
		...
		function SectionFunctions:Tab(Settings)
			...
			if Settings.Image then
				...
				tabImage.Image = SonLib:ResolveImage(Settings.Image, tabImage, "Image")
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Button(Settings, Flag)
					...
					if Flag then
						SonLib.Options[Flag] = ButtonFunctions
					end
				...
				function SectionFunctions:Toggle(Settings, Flag)
					...
					if Flag then
						SonLib.Options[Flag] = ToggleFunctions
					end
				...
				function SectionFunctions:Slider(Settings, Flag)
					...
					if Flag then
						SonLib.Options[Flag] = SliderFunctions
					end
				...
				function SectionFunctions:Input(Settings, Flag)
					...
					if Flag then
						SonLib.Options[Flag] = InputFunctions
					end
				...
				function SectionFunctions:Keybind(Settings, Flag)
					...
					if Flag then
						SonLib.Options[Flag] = KeybindFunctions
					end
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					if Flag then
						SonLib.Options[Flag] = DropdownFunctions
					end
				...
				function SectionFunctions:Colorpicker(Settings, Flag)
					...
					if Flag then
						SonLib.Options[Flag] = ColorpickerFunctions
					end
				...
				function SectionFunctions:Header(Settings, Flag)
					...
					if Flag then
						SonLib.Options[Flag] = HeaderFunctions
					end
				...
				function SectionFunctions:Label(Settings, Flag)
					...
					if Flag then
						SonLib.Options[Flag] = LabelFunctions
					end
				...
				function SectionFunctions:SubLabel(Settings, Flag)
					...
					if Flag then
						SonLib.Options[Flag] = SubLabelFunctions
					end
				...
				function SectionFunctions:Paragraph(Settings, Flag)
					...
					if Flag then
						SonLib.Options[Flag] = ParagraphFunctions
					end
			...
			function TabFunctions:InsertConfigSection(Side)
				...
				local configSelection = configSection:Dropdown({
					...
					Multi = false,
					Required = false,
					Options = SonLib:RefreshConfigList(),
				...
				configSection:Button({
					...
					Callback = function()
						...
						local success, returned = SonLib:SaveConfig(inputPath)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
						...
						configSelection:InsertOptions(SonLib:RefreshConfigList())
					end,
				})
				...
				configSection:Button({
					Name = "Load Config",
					Callback = function()
						local success, returned = SonLib:LoadConfig(configSelection.Value)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
				...
				configSection:Button({
					...
					Callback = function()
						local success, returned = SonLib:SaveConfig(configSelection.Value)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
				...
				configSection:Button({
					...
					Callback = function()
						...
						writefile(SonLib.Folder .. "/settings/autoload.txt", name)
				...
				if isfile(SonLib.Folder .. "/settings/autoload.txt") then
					local name = readfile(SonLib.Folder .. "/settings/autoload.txt")
					autoloadLabel:UpdateName("Autoload config: " .. name)
				end
			end
	...
	function WindowFunctions:Unload()
		...
		for _, wm in ipairs(SonLib._watermarks) do
			wm:Destroy()
		end
		SonLib._watermarks = {}
		sonGui:Destroy()
	...
	local ClassParser = {
		["Toggle"] = {
			...
			Load = function(Flag, data)
				if SonLib.Options[Flag] and data.state then
					SonLib.Options[Flag]:UpdateState(data.state)
				end
			end
		},
		["Slider"] = {
			...
			Load = function(Flag, data)
				if SonLib.Options[Flag] and data.value then
					SonLib.Options[Flag]:UpdateValue(data.value)
				end
			end
		},
		["Input"] = {
			...
			Load = function(Flag, data)
				if SonLib.Options[Flag] and data.text and type(data.text) == "string" then
					SonLib.Options[Flag]:UpdateText(data.text)
				end
			end
		},
		["Keybind"] = {
			...
			Load = function(Flag, data)
				if SonLib.Options[Flag] and data.bind then
					SonLib.Options[Flag]:Bind(Enum.KeyCode[data.bind])
				end
			end
		},
		["Dropdown"] = {
			...
			Load = function(Flag, data)
				if SonLib.Options[Flag] and data.value then
					SonLib.Options[Flag]:UpdateSelection(data.value)
				end
			end
		},
		["Colorpicker"] = {
			...
			Load = function(Flag, data)
				...
				if SonLib.Options[Flag] and data.color then
					SonLib.Options[Flag]:SetColor(HexToColor3(data.color)) 
					if data.alpha then
						SonLib.Options[Flag]:SetAlpha(data.alpha)
					end
				end
			end
		}
	...
	function SonLib:SetFolder(Folder)
		if isStudio then return "Config system unavailable." end

		SonLib.Folder = Folder;
		BuildFolderTree()
	end

	function SonLib:SaveConfig(Path)
		...
		local fullPath = SonLib.Folder .. "/settings/" .. Path .. ".json"

		local data = {
			objects = {}
		}

		for flag, option in next, SonLib.Options do
	...
	function SonLib:LoadConfig(Path)
		if isStudio or not (isfile and readfile) then return "Config system unavailable." end
		...
		local file = SonLib.Folder .. "/settings/" .. Path .. ".json"
	...
	function SonLib:RefreshConfigList()
		if isStudio or not (isfolder and listfiles) then return "Config system unavailable." end

		local list = (isfolder(SonLib.Folder) and isfolder(SonLib.Folder .. "/settings")) and listfiles(SonLib.Folder .. "/settings") or {}
	...
	table.insert(SonLib.themeCallbacks, function(newName)
		local newTheme = SonLib.Themes[newName]
...
function SonLib:Watermark(Settings)
	...
	gui.Parent = gethui and gethui() or SonLib.GetService("CoreGui")
	...
	table.insert(SonLib._watermarks, WatermarkFunctions)
	...
	function WatermarkFunctions:Destroy()
		gui:Destroy()
		for i, wm in ipairs(SonLib._watermarks) do
			if wm == self then
				table.remove(SonLib._watermarks, i)
				break
			end
		end
	end
